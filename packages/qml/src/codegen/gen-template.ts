...
function nodeToHtml(
  ...
  const ownAttrs = computeAttributes(node, options.controllerName, mapping);

  if (node.type === "Text") {
    const text = node.properties.text;
    if (typeof text === "string") {
      return `${indent}<span class="${ownClasses}">${escapeHtml(text)}</span>`;
    }
    if (text && typeof text === "object" && text.type === "binding") {
      const expr = translateBinding(text.expression, options.controllerName);
      return `${indent}<span class="${ownClasses}">{{ ${expr} }}</span>`;
    }
  }
...
function generateRepeater(
  ...
  if (typeof model === "object" && model.type === "binding") {
    modelExpr = translateBinding(model.expression, options.controllerName);
  } else if (typeof model === "string") {
    modelExpr = translateBinding(model, options.controllerName);
  }
...
function computeAttributes(
  ...
  Object.entries(node.signalHandlers).forEach(([key, value]) => {
    ...
    const method = translateHandler(value, controllerName);
    attrs.push(`(${eventName})="${method}"`);
  });
  ...
  for (const [prop, value] of Object.entries(node.properties)) {
    ...
    if (prop === "text" && node.type !== "Text") {
      if (typeof value === "string") {
        attrs.push(`text="${escapeAttr(value)}"`);
      } else if (value && typeof value === "object" && value.type === "binding") {
        attrs.push(`[text]="${translateBinding(value.expression, controllerName)}"`);
      }
      continue;
    }
...
export function translateBinding(expr: string, controllerName: string): string {
  let result = expr;

  result = result.replace(/(\w+)\.get\("(\w+)"\)/g, "$1.$2");

  result = result.replace(/controller\.(\w+)\.value\b/g, (_, prop) => {
    return `${controllerName}.${prop}()`;
  });
  ...
  result = result.replace(/(\w+(?:State|Service)?)\.get\("(\w+)"\)/g, (_, svc, prop) => {
...
function translateHandler(value: string, controllerName: string): string {
  ...
  if (value.includes("controller.")) {
    return value.replace(/controller\./g, `${controllerName}.`).replace(/\(\)/g, "()");
  }
  return value;
}
