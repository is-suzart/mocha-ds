  ...
  template: `
    @for (pos of positions; track pos) {
      @if (groupedToasts(pos); as items) {
        <div [class]="'toast-container toast-container--' + pos">
          @for (item of items; track item.id) {
            <div
              [class]="'toast toast--' + item.variant + (item.filled ? ' toast--filled' : '') + (item.color ? ' toast--color-' + item.color : '') + (item.exiting ? ' toast--exiting' : '') + (item.className ? ' ' + item.className : '')"
              [style]="item.style"
              role="alert"
              aria-live="assertive"
            >
