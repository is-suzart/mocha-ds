...
export class TreeTableComponent {
  ...
  emptyState = input<string>('Nenhum registro encontrado.');
  ...
  expandedRowIds = input<(string | number)[]>([]);
  selectedRowIds = input<(string | number)[] | undefined>(undefined);

  sortFieldChange = output<string>();
  sortOrderChange = output<'asc' | 'desc' | ''>();
  expandedRowIdsChange = output<(string | number)[]>();
  selectedRowIdsChange = output<(string | number)[]>();

  // Local fallbacks
  protected localSortField = signal<string>('');
  protected localSortOrder = signal<'asc' | 'desc' | ''>('');
  protected localExpandedIds = signal<Set<string | number>>(new Set());
  protected localSelectedIds = signal<(string | number)[]>([]);

  // Active states resolved between inputs and local fallback signals
  protected activeSortField = computed(() => this.sortField() !== '' ? this.sortField() : this.localSortField());
  protected activeSortOrder = computed(() => this.sortOrder() !== '' ? this.sortOrder() : this.localSortOrder());
  protected activeExpandedIds = computed(() => {
  ...
  protected activeSelectedIds = computed(() => this.selectedRowIds() !== undefined ? this.selectedRowIds()! : this.localSelectedIds());
  protected activeSelectedSet = computed(() => new Set<string | number>(this.activeSelectedIds()));
  ...
  protected colspan = computed(() => this.columns().length + (this.showCheckbox() ? 1 : 0));
  protected skeletonRows = computed(() => Array.from({ length: this.loadingRowsCount() }, (_, i) => i));
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (query && data.length > 0 && fields.length > 0) {
        ...
        const findMatches = (nodes: any[], path: (string | number)[]) => {
          nodes.forEach(node => {
  ...
  protected visibleRows = computed(() => {
    return this.flattenTreeNodes(
      ...
      null,
      [],
      this.activeExpandedIds()
    );
  });

  // Checkbox state calculations
  protected totalVisibleNodesCount = computed(() => {
    let count = 0;
  ...
  protected allRowsSelected = computed(() => {
    if (this.totalVisibleNodesCount() === 0) return false;
  ...
  protected someRowsSelected = computed(() => {
    if (this.totalVisibleNodesCount() === 0 || this.activeSelectedSet().size === 0) return false;
    let selectedCount = 0;
    const countSelected = (nodes: any[]) => {
      nodes.forEach(node => {
        if (this.activeSelectedSet().has(this.rowKey()(node))) {
          selectedCount++;
        }
  ...
  private filterTreeData(data: any[], query: string, fields: string[]): any[] {
  ...
  private sortTreeData(data: any[], field: string, order: 'asc' | 'desc' | ''): any[] {
    const sorted = [...data];
  ...
  private getAllChildIds(node: any): (string | number)[] {
    const children = node[this.childrenKey()] as any[] | undefined;
  ...
  private updateSelectionCascading(
    ...
    const selectedSet = new Set<string | number>(this.activeSelectedIds());
    ...
    if (!targetNode) return this.activeSelectedIds();
    ...
    >();
    const buildParentMap = (nodes: any[], parentId: string | number | null) => {
  ...
  // Event handlers
  protected handleHeaderClick(column: TreeColumn) {
  ...
  protected toggleExpand(row: any) {
  ...
  protected handleSelectRow(row: any, event: Event) {
    ...
    if (this.cascadeSelection()) {
      ...
      const nextSet = new Set<string | number>(this.activeSelectedIds());
      if (checked) {
        nextSet.add(rowId);
      } else {
        nextSet.delete(rowId);
